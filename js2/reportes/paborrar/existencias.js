...
async function cargarFiltrosExistencias() {
    try {
        ...
        _poblarSelect('filtroMedicamentoExistencia', medicamentos,  'id', 'nombre', 'Todos');
        _poblarSelect('filtroPresentacionExistencia', presentaciones, 'id', 'nombre', 'Todas');
...
function _poblarSelect(selectId, items, keyVal, keyLabel, placeholderLabel) {
...
function _poblarSelectLotes(lotes) {
    ...
    if (!sel) return;
    sel.innerHTML = '<option value="">Todes</option>';
    lotes.forEach(l => {
        const opt = document.createElement('option');
        opt.value       = l.id;
